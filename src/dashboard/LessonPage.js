...
export const LessonPage = () => {
    ...
    return (
      <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center">
        <div className="text-center p-8">
          <p className="text-gray-600 mb-4">Lesson content for "{decodedTopic}" is coming soon.</p>
          <button onClick={() => navigate('/dashboard')}
            className="px-6 py-3 bg-[#1a365d] text-white rounded-xl font-bold"
          >
            Back to Dashboard
          </button>
        </div>
  ...
  const handleDiscuss = () => {
    // Pass subject and topic to Chat via location.state
    navigate('/chat', { state: { subject: decodedSubject, topic: decodedTopic } });
  };

  const handleQuiz = () => {
    // Navigate to quiz using URL params
    navigate(`/quiz/${encodeURIComponent(decodedSubject)}/${encodeURIComponent(decodedTopic)}`);
  };
      ...
      <div className="max-w-3xl mx-auto px-5 py-8 space-y-8">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-3">
          ...
          {lesson.detailedContent.split('\n').map((para, idx) =>
            para.trim() ? <p key={idx} className="text-gray-700 leading-relaxed text-base">{para.trim()}</p> : null
          )}
        </motion.section>
        ...
        >
          {/* Navigates to /chat with subject+topic as state */}
          ...
          {/* Navigates to /quiz/:subject/:topic */}
